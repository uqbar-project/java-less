		...
		"should succeed" - {
			...
			"for methods" - {
				...
				"with one argument but no body" in {
					...
					"""(argumentedEmptyMethod -> 0.until(4), argumentedEmptyMethod.name -> 0.until(1), argumentedEmptyMethod.arguments -> 1.to(3), "arg" -> 2.until(3))
				}
				...
				"with arguments but no body" in {
					...
					"""(argumentedEmptyMethod -> 0.until(6), argumentedEmptyMethod.name -> 0.until(1), argumentedEmptyMethod.arguments -> 1.to(5), "arg1" -> 2.until(3), "arg2" -> 4.until(5))
				}
